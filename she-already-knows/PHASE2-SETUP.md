...
1. Swap Stripe test keys → live keys; recreate the Product/price + webhook in live mode and update the env vars. Ensure the **Terms of Service URL is set in live mode** (Stripe → Settings → Checkout) — checkout requires it (`consent_collection`). Also create a **live Customer Portal configuration** (Stripe → Settings → Billing → Customer portal → activate, enabling cancel + payment-method update + invoice history) so the "Manage subscription" link works in live mode.
